...
- =~/.config/gnaw/config.edn= (shared with gnaw), containing at least
  a =:sources= vector with =:urls= entries pointing to =reports.json=
  files (local paths or URLs).
- =notmuch-gnaw= does not call the =gnaw= binary, it only reads the
  files that =gnaw= produces (=M-x gnaw-add-source= can create
  =config.edn= for you).
...
=~/.config/gnaw/state.edn=, which is shared with the =gnaw= CLI,
=M-x gnaw= and the other mail front-ends, so marks are visible across
all clients.

- =M-x notmuch-gnaw-mark-sticky= -- toggle the sticky mark (keep visible).
- =M-x notmuch-gnaw-mark-dismiss= -- toggle the dismiss mark (hide).
